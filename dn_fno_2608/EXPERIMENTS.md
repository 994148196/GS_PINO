# dn_fno_2608 改进实验总览（exp001–exp009）
...
跨位形泛化，专职模型跨配置外推崩溃**（DN→SN 253%、SN→DN 4.0e8%）。
...
| 位形泛化 | data_v5（MAST DN+SN）+ config 通道（exp008/009） | **混合训练以 +26~35% 代价换来跨位形泛化；专职跨配置外推崩溃** |
...
| exp008 | data_v5 | 混合 DN+SN：X点+锚点+config 14ch | DN桶 0.667% / SN桶 1.203% / 整体 0.935% | —（跨机器，不与 N=500 基线直接比） | **混合训练可行**：config 通道被利用，两桶都健康（GS 0.986、find_critical 0/1000）；vs 专职代价 +26~35% |
...
5. **N=500 对"信息完整 + 目标物理一致"的数据足够**：v4 的残余差距
   ...
   数据量不足。
6. **跨位形泛化需要混合训练（data_v5/exp008/009）**：config 通道（1ch，
   one-hot z-score）让混合模型在 DN/SN 间切换映射——整体 test 0.935%、
   两桶都健康；代价 vs 专职 +26~35%（DN 桶 0.667 vs 0.492、SN 桶 1.203 vs
   0.953）。**专职模型跨配置外推完全失败**（DN→SN 253%、SN→DN 4.0e8%）：
   FNO 是分布内插值器，训练分布覆盖是必要条件（与结论 1"数据物理合理性
   > 输入架构"同构：**训练分布覆盖 > 架构**）。
7. **SN 桶误差系统性高于 DN 桶**（专职 0.953 vs 0.492，混合同趋势）：SN
...
- **诊断字段下游**：data_v4 已落盘 8 个约束诊断字段（实际 X 点、O 点、
  ...
  （把超定系统不可达信息作为输入/目标）。
- **位形泛化扩展**：data_v5 架构（machine×config 注册表 + config 通道 +
  多文件拼接）为 snowflake/limiter 预留（v6：freegs_snow fork 二阶约束 /
  DIIID 两步法，需全链域适配）；第三位形加入混合池是下一自然步。
- **混合代价归因**：+26~35% 是共享容量代价还是 config 通道信息瓶颈？
  可用 N=2000 或分位形独立归一化（每桶自己 z-score）消融。
...
| exp001–exp009/ | 每实验 README（设计/结果/结论）+ notes（解读/偏差记录）+ metrics.json |
