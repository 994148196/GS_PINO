...
"$PY" -m gs_pino.generate_dn_dataset --split val --n-samples 500 --seed 456 \
    --out-dir "$OUT" --chunk-size 500 --n-jobs "$JOBS" | tee dn_fno_2608/logs/gen_val.log

echo "=== generating test (500, seed 789) ==="
"$PY" -m gs_pino.generate_dn_dataset --split test --n-samples 500 --seed 789 \
...
echo "=== merging chunks ==="
"$PY" -m gs_pino.generate_dn_dataset --split val   --out-dir "$OUT" --merge
"$PY" -m gs_pino.generate_dn_dataset --split test  --out-dir "$OUT" --merge
"$PY" -m gs_pino.generate_dn_dataset --split train --out-dir "$OUT" --merge
