#!/usr/bin/env bash
# exp012: coil 21ch 混合训练（MASTU_simple 五配置）评估 + 可视化
# 6 桶：eval_all（5 文件 test pool，n=1000）+ 5 单配置各 200
# limiter 桶：X 点/分离面几何指标 NaN（无分离面），只报 rel L2/RMSE/GS 残差/O 点
# 用法: bash dn_fno_2608/scripts/run_exp012_eval_vis.sh
set -euo pipefail
cd "$(git rev-parse --show-toplevel)"
PY="C:/Users/HP/.conda/envs/torch5060/python.exe"
V6="dn_fno_2608/data_v6"
EXP="dn_fno_2608/experiments/exp012_coil_input_v6"
CKPT="$EXP/best.pt"

ALL="$V6/dn/test.npz,$V6/sn/test.npz,$V6/snow_single/test.npz,$V6/snow_double/test.npz,$V6/limiter/test.npz"
...
for cfg in dn sn snow_single snow_double limiter; do
    ...
    "$PY" -u -m gs_pino_dn_fno_2608.evaluate_dn_fno \
      --test-data "$V6/$cfg/test.npz" --checkpoint "$CKPT" \
...
for cfg in dn sn snow_single snow_double limiter; do
    "$PY" -u -m gs_pino_dn_fno_2608.visualize_dn_fno \
      ...
      --title "exp012 coil 21ch mixed 5-config (N=500) - $cfg test" 2>&1 | tail -3
done
echo "== done =="
