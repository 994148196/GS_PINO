...
| [exp107_pure_data_dn500](exp107_pure_data_dn500/README.md) | data_v5/dn 18ch coil（exp011 口径去掉 SN：纯 MSE 直出 psi_total，仅 500 DN 样本） | rel_l2_pct **0.799%**（median 0.666 / p95 1.72）；X 点 2.16/1.46 cm；O 点 0.44；分离面 0.50；GS 残差（芯部）**3.07×** | exp011 DN 桶（旧表2 基线 0.841% / GS 芯部 3.79×）；exp101（0.715% / 3.31×）；exp102（0.800% / 3.75×） | **论文表2 公平性修正基线**：旧表2 误用混合训练 exp011 的 DN 桶当纯数据基线；DN-only 基线 rel L2 更优（0.799 vs 0.841）且 X 点显著更好（2.16/1.46 vs 3.16/2.23 cm）——混合训练代价主要在几何定位；物理残差增益保持（单阶段 −10%、两阶段持平但多出自洽）；**同时修正表2 GS 残差行口径混用**（几何 mask 1.01×/0.99× 不可比 → 统一芯部 mask：基线 3.07×/3.79× 与方案 3.31×/3.75× 同量级）；表2 基线列换用本实验、表3 混合对比保留 exp011 |
...
| [exp305_ufno_tuned_pino_twostage_n500](exp305_ufno_tuned_pino_twostage_n500/README.md) | data_v5/dn 18ch coil（同 exp102 口径） | rel_l2_total **0.7147%**（plasma 0.81%）；Ip 0.182%；J mask 1.20%；X 点 0.63/0.61 cm | exp302（UFNO 0.729%） | **UFNO 架构调优边际小胜**：解码器谱化 + bottleneck×2 + 65² 模态 20×20（3.60M 仍 < FNO 4.21M）→ 总误差 −2%、Ip −14%，J/X 略退，全在单 seed 噪声内；**最显著行为收益是阶段1 收敛翻倍（切换 e30 vs e70）**——exp302 架构已接近本数据上限，改进不在骨架侧 |
| [exp306_ufno_weights_pino_twostage_n500](exp306_ufno_weights_pino_twostage_n500/README.md) | data_v5/dn 18ch coil（同 exp102 口径） | rel_l2_total **0.7794%**（plasma 0.88%）；Ip 0.240%；J mask 1.14% | exp302（UFNO 0.729%） | **UFNO 训练侧调优负收益（零代码差分）**：w_j 2.0/w_pde 0.3/阈值 4%/ramp 60 → 总误差 +0.05pp、plasma +0.06pp（权重挤压 ψ 主任务，§6 预判应验）；4% 阈值成功提前切换（e31）但无净收益——exp102 权重杠杆已吃满 |
| [exp307_pideeponet_wide_pino_twostage_n500](exp307_pideeponet_wide_pino_twostage_n500/README.md) | data_v5/dn 18ch coil（同 exp102 口径） | rel_l2_total **0.7575%**（plasma 0.87%）；Ip 0.210%；J mask 2.13%；X 点 0.59/0.51 cm | exp304（DeepONet 0.802%） | **DeepONet 加宽验证容量诊断**：256→384（1.34M）→ 总误差 −5.6%（低于 FNO 0.800%）、**Ip −45% 反超 UFNO**、J −0.48pp、X 点 −19%/−27%——exp304 "J/Ip 弱是容量不足"被证实；1.34M 参数超越 4.2M FNO，分支-主干路线第二条可行解 |
| [exp308_pideeponet_ff_pino_twostage_n500](exp308_pideeponet_ff_pino_twostage_n500/README.md) | data_v5/dn 18ch coil（同 exp102 口径） | **失败**：阶段2 崩溃（pde 22.0 vs 6.6e-4、val 2.89%→92.6%），e138 正确早停；artifact 为阶段1 模型（test 2.539% 无意义） | exp307（0.7575%） | **trunk 傅里叶特征与逐点 FD 物理残差不兼容（诚实负面结果）**：γ(R,Z) 高频通道（32π）注入空间高频 → 二阶差分放大 → 物理项梯度风暴；排他证据：exp307 同配置无 FF 正常、自测/冒烟有限——非 bug，是机制性不兼容；修复方向：FF 小缩放/ff_l 减小/SIREN 可学习频率 |
| [exp311_tkno_lite_pino_twostage_n500](exp311_tkno_lite_pino_twostage_n500/README.md) | data_v5/dn 18ch coil（同 exp102 口径） | rel_l2_total **0.635%**（plasma 0.716%）；Ip 0.189%；J mask 0.977%；X 点 0.42/0.43 cm；O 点 0.18 cm | exp305（UFNO tuned 0.7147%）/ exp302（UFNO 0.729%） | **TKNO-lite（Transformer 全局注意力，去 KAN）全面刷新系列纪录**：1.21M 参数（FNO 0.29×）下总误差 −11%、J −14%（0.977% 系列最佳）、X/O 点定位全系列最佳——**磁面全局耦合的决定变量是混合的全局性而非傅里叶实现**（对照 exp301 UNet 失败）；同任务文献锚点 arXiv:2511.19114（ICML 2026）；唯一代价：80 min 训练（本构建无 flash 注意力） |
| [exp312_pod_deeponet_pino_twostage_n500](exp312_pod_deeponet_pino_twostage_n500/README.md) | data_v5/dn 18ch coil（同 exp102 口径） | rel_l2_total **1.508%**（plasma 1.69%）；Ip 0.253%；J mask 2.52%；X 点 0.85/0.85 cm | exp307（DeepONet wide 0.7575%） | **低秩先验过度激进（诚实负面）**：POD 基 p_psi=3（99.92% 能量）/p_j=10——ψ 解空间极低秩（数据集结构诊断），但截断的 0.08% 恰是 X 点尺度细节：总误差 2×、X/O 点 +0.3~0.45 cm；Ip 0.253% 证明物理约束在低秩空间仍有效；**"数据低秩 ⇒ 线性基够用"被否定**——学习 trunk（exp307）表达超越 SVD 线性子空间 |
| [exp313_pod_residual_pino_twostage_n500](exp313_pod_residual_pino_twostage_n500/README.md) | data_v5/dn 18ch coil（同 exp102 口径） | rel_l2_total **0.883%**（plasma 1.03%）；Ip 0.338%；J mask 1.76%；X 点 0.54/0.56 cm；O 点 0.34 cm | exp312（POD 1.508%）/ exp102（FNO 0.800%） | **POD 基 + 小 FNO 残差混合（正面，exp312 诊断的落地修复）**：残差通道（870K = FNO 0.21×）补上 POD 基外细节——总误差 −41%（1.508→0.883%）、X 点 −36%/−34%、O 点 −48%、J −30%、GS 残差 −73%；分阶段冻结协议（先 branch 60 epochs 后残差）消除通道竞争，switch_epoch 61 = pretrain 结束立即切；vs 纯 FNO 仍差 0.08pp（median 持平）但 **X 点反超 FNO**，低秩基 + 谱残差以 1/5 参数达 FNO ~90% 精度；Ip 0.338% 略退化（残差放大 J 自由度，分布更准积分略松） |
| [exp314_pod_residual_unet_pino_twostage_n500](exp314_pod_residual_unet_pino_twostage_n500/README.md) | data_v5/dn 18ch coil（同 exp102 口径） | rel_l2_total **1.099%**（plasma 1.28%）；Ip 0.403%；J mask 2.33%；X 点 0.64/0.64 cm；O 点 0.36 cm | exp313（POD+FNO 0.883%）/ exp301（UNet 全任务 2.202%） | **基外细节"部分需要"全局混合（部分正面）**：纯卷积 UNet 残差把 POD 从 1.508% 拉到 1.099%（−27%，X 点 −25%、O 点 −44%、GS 残差 −62%）——POD 基承担全局形状后残差通道的全局性需求大幅下降（完整任务 2.75× 差距 → 残差任务 1.25×）；但谱混合仍有边际增益（1.099 vs 0.883，+0.22pp，X 点差 0.1 cm）；混合框架稳健：即使残差用系列最弱架构也保底 −27%；UNet 残差收敛慢（best e796 跑满 800） |
