...
> 生成日期：2026-08-14 ｜ 代码：`gs_pino_dn_fno_2608.generate_dn_dataset`
> （新增可行区采样 / 约束检查 CLI 开关）
> 用途：exp006（X点/锚点输入）/ exp007（coil 输入）数据；修复 data_v3 的
> isoflux 约束不可达缺陷（残差 mean 0.50/max 2.10 → mean 0.172/max ≤0.35）
> 取代 data_v3（已删除；复现脚本 `scripts/run_generate_v3.sh` 保留）
> 一键复现：`bash dn_fno_2608/scripts/run_generate_v4.sh`
...
| 输入通道 | 三种模式：**11ch**（X点，exp006-A）/ **13ch**（X点+锚点，exp006-A'）/ **11ch**（coil，exp007-B）（§4） |
| 目标 | `psi_total`（65²，z-score） |
| 求解器 | freegs 0.8.3.dev54：双 X 点 + isoflux，γ=1e-12；rtol=1e-3、maxits=50（同 data/） |
| 新增 | 可行区采样 + **7 项接受约束** + `anchor` + 7 个约束诊断字段（`--save-constraint-diag`） |
| 核心结果 | **约束不可达是 exp004/005 退化主因**：A 9.2×、A' 49×、B 25× 恢复（exp006/007） |
...
## 3. 字段说明（npz keys，25 个）

data_v2 全部 17 字段 + `anchor` + 7 个约束诊断字段（仅 `--save-constraint-diag`
时生成，merge 对缺字段容错）：
...
| `x_coords` | (N,4) | [R_lo, Z_lo, R_up, Z_up] X 点目标坐标 | 通道 8–11（11ch X点模式） |
| `anchor` | (N,2) | isoflux 锚点 [R,Z]，中平面 Z≡0、R~U[1.35,1.65] | 通道 12–13（13ch xa 模式） |
...
| `xpts_actual` | (N,2,3) | find_critical 实际 X 点 [R,Z,psi]，按目标 lo/up 贪婪配对排序 | 否（评估几何真值） |
| `o_point` | (N,3) | 实际 O 点 [R,Z,psi] | 否 |
| `xpt_constraint_res` | (N,4) | 目标处 Br_lo, Bz_lo, Br_up, Bz_up（T）——X 点约束残差 | 否（诊断） |
| `isoflux_res` | (N,2) | psi(lo)−psi(锚点), psi(up)−psi(锚点)（Wb） | 否（诊断） |
| `psi_at_constraints` | (N,3) | psi(lo), psi(up), psi(锚点)（Wb） | 否（诊断） |
| `n_iter` | (N,1) | Picard 迭代数 | 否（诊断） |
| `psi_relchange_final` | (N,1) | 末次 psi 相对变化（收敛样本 ≤ rtol=1e-3） | 否（诊断） |
...
通道顺序 = 代码实际拼接顺序（`data_dn_fno.py` / `data_dn_fno_coils.py`）：
R/Z + params + x_coords [+ anchor]（coil 模式：R/Z + params + coil_currents）。
R/Z 线性 [-1,1]；标量 z-score（训练集统计）。A' 的锚点 Z≡0 通道 std=0，数据集类
分母 `np.maximum(std, 1e-8)` 防护 → 常量通道映射为 0。

| # | 11ch X点（exp006-A） | 13ch X点+锚点（exp006-A'） | 11ch coil（exp007-B） |
|---|---|---|---|
| 1–2 | R, Z 网格 | R, Z 网格 | R, Z 网格 |
| 3–7 | Ip, Paxis, fvac, alpha_m, alpha_n | 同左 | 同左 |
...
- 参数范围同 data_v2：Ip U[5e4,4e5] A、Paxis U[200,3000] Pa、fvac U[0.5,3.0]、
  alpha_m U[1,2]、alpha_n U[1.5,2.5]

### 7.2 7 项接受约束（全部在 `_solve_one` 内，每项一个 CLI 开关）
...
### 7.3 阈值校准（用已知健康数据 data_v2 的 500 val 样本）
...
1. **与 data_v3 差异**：X 点中心 (1.1,±0.6)→(1.2,±0.6)、抖动 ±0.20→R±0.10/Z±0.15、锚点固定→中平面采样 R∈[1.35,1.65]、+7 项接受约束、+8 诊断字段；
2. 初版四边形余量 0.10 m + tokamak 线圈顺序多边形（自交叉蝴蝶结，距离算错）→ 98% 误拒：修复为极角凸包排序 + 余量 0.05（§7.3）；
3. isoflux 残差阈值 0.25 → 0.35（v2 p95 校准）；
4. 所有新 flag 默认关（v3 命令逐字节复现，回归验证通过）；
5. 诊断字段仅 `--save-constraint-diag` 时保存；
6. **三模型复测结果**（exp006/007，N=500 seed 1，test 494）：A（X点 11ch）31.09%→**3.38%**（9.2×）、A'（X点+锚点 13ch）21.72%→**0.442%**（49×）、B（coil 11ch）12.47%→**0.499%**（25×）——**约束不可达是 exp004/005 退化主因**；
7. A' 训练 NaN 修复：锚点 Z≡0 → z-score 0/0 → 分母 `np.maximum(std, 1e-8)`（data_dn_fno.py / data_dn_fno_coils.py，data_v2/v3 路径不受影响）。
