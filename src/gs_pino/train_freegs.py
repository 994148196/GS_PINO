...
def run_epoch(
    ...
    return {
        ...
        "total": (total_mse + total_pde) / n_samples,
    }
...
def main() -> None:
    ...
    parser.add_argument("--hidden-dim", type=int, default=128, help="Hidden dimension for PlaNetCore.")
    parser.add_argument("--scale-mse", type=float, default=1.0, help="Weight for MSE loss.")
    parser.add_argument("--scale-pde", type=float, default=0.1, help="Weight for PDE loss.")
    ...
    parser.add_argument("--amp", action=argparse.BooleanOptionalAction, default=False, help="Enable mixed precision.")
    parser.add_argument("--warmup-epochs", type=int, default=10, help="Number of warmup epochs.")
    parser.add_argument("--patience", type=int, default=50, help="Early stopping patience.")
    parser.add_argument("--min-epochs", type=int, default=100, help="Minimum training epochs before early stopping.")
    ...
    print(f"  Loss weights: mse={args.scale_mse}, pde={args.scale_pde}")
    print(f"{'='*60}\n")
    ...
    loss_module = PlaNetLoss(is_physics_informed=True, scale_mse=args.scale_mse, scale_pde=args.scale_pde)
    ...
    for epoch in pbar:
        ...
        train_losses = run_epoch(model, train_loader, opt, device, loss_module, clip_grad=args.clip_grad, amp=args.amp, accum_steps=args.accum_steps, pde_scale=pde_scale)
        val_losses = run_epoch(model, val_loader, None, device, loss_module, pde_scale=pde_scale) if len(val_ds) else train_losses
        ...
        pbar.set_postfix(
            mse=f"{train_losses['mse']:.4f}",
            pde=f"{train_losses['pde']:.4f}",
            pde_scale=f"{pde_scale:.2f}",
...
def plot_training_history(history: list[dict], output_dir: Path) -> None:
    ...
    ax = axes[0, 2]
    ax.plot(epochs, [h["train_mse"] for h in history], "b-", label="Train")
    ax.plot(epochs, [h["val_mse"] for h in history], "r--", label="Val")
    ax.set_xlabel("Epoch")
    ...
    ax.set_xlabel("Epoch")
    ax.set_ylabel("PDE Loss")
    ax.set_title("PDE Residual Loss (Log Scale)")
    ax.set_yscale("log")
    ax.legend()
    ax.grid(alpha=0.3)
