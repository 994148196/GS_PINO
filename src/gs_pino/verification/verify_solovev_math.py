"""Mathematical verification that Solov'ev solution satisfies GS equation."""

import numpy as np
import matplotlib.pyplot as plt
from pathlib import Path

PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent.parent
OUTPUT_DIR = PROJECT_ROOT / "outputs"


def solovev_psi_bar(rho, alpha_m=1.0, alpha_n=2.0, Beta0=0.5):
    """Solov'ev normalized psi."""
    core = np.clip(1.0 - rho**alpha_m, 0.0, None)
    psi_bar_raw = core * (1.0 + Beta0 * core**alpha_n)
    psi_bar = psi_bar_raw / (1.0 + Beta0)
    return psi_bar


def verify_solovev_math():
    """Verify Solov'ev satisfies GS equation mathematically.
    
    Grad-Shafranov equation in cylindrical coordinates:
        -∇²ψ = μ₀ Jtor
    
    where Jtor = R * p'(ψ) + FF'(ψ) / (μ₀ R)
    
    Using gspack's parameterization:
        p'(ψN) = L * Beta0 / R0 * (1 - ψN^αm)^αn
        FF'(ψN) = μ₀ * L * (1 - Beta0) * R0 * (1 - ψN^αm)^αn
    
    For Solov'ev: ψ = ψ_axis * ψ_bar(rho)
    """
    R0 = 1.0
    a = 0.5
    Ip = 2e5
    Beta0 = 0.5
    alpha_m = 1.0
    alpha_n = 2.0
    
    mu0 = 4 * np.pi * 1e-7
    L = Ip / (2 * np.pi * R0)
    
    psi_axis = mu0 * L * R0**2 / 2 * (1.0 + Beta0)
    
    nr = 500
    rho = np.linspace(1e-6, 0.999, nr)
    drho = rho[1] - rho[0]
    
    psi_bar = solovev_psi_bar(rho, alpha_m, alpha_n, Beta0)
    
    dpsi_drho = np.gradient(psi_bar, drho)
    d2psi_drho2 = np.gradient(dpsi_drho, drho)
    
    lap_psi_bar_rho = (1.0 / rho) * dpsi_drho + d2psi_drho2
    
    lap_psi = lap_psi_bar_rho * psi_axis / a**2
    
    psiN = psi_bar
    
    jtorshape = (1.0 - psiN**alpha_m)**alpha_n
    
    pprime_val = L * Beta0 / R0 * jtorshape
    ffprime_val = mu0 * L * (1.0 - Beta0) * R0 * jtorshape
    
    Jtor = R0 * pprime_val + ffprime_val / (mu0 * R0)
    
    rhs = mu0 * Jtor
    
    residual = -lap_psi - rhs
    
    print(f"Mathematical Verification of Solov'ev Equilibrium:")
    print(f"=" * 60)
    print(f"  Parameters:")
    print(f"    R0 = {R0} m")
    print(f"    a = {a} m")
    print(f"    Ip = {Ip} A")
    print(f"    Beta0 = {Beta0}")
    print(f"    alpha_m = {alpha_m}")
    print(f"    alpha_n = {alpha_n}")
    print(f"    L = {L:.6e}")
    print(f"    psi_axis = {psi_axis:.6e} Wb/rad")
    print(f"=" * 60)
    print(f"  GS Residual Statistics:")
    print(f"    Mean: {np.mean(residual):.6e}")
    print(f"    Max: {np.max(residual):.6e}")
    print(f"    Min: {np.min(residual):.6e}")
    print(f"    RMSE: {np.sqrt(np.mean(residual**2)):.6e}")
    print(f"    Relative RMSE: {np.sqrt(np.mean(residual**2)) / np.sqrt(np.mean(rhs**2)):.6e}")
    print(f"=" * 60)
    
    fig, axes = plt.subplots(2, 2, figsize=(12, 10))
    
    axes[0, 0].plot(rho, psi_bar, 'b-')
    axes[0, 0].set_title('psi_bar')
    axes[0, 0].set_xlabel('rho')
    
    axes[0, 1].plot(rho, lap_psi_bar_rho, 'r-')
    axes[0, 1].set_title('Laplacian of psi_bar (rho coords)')
    axes[0, 1].set_xlabel('rho')
    
    axes[1, 0].plot(rho, rhs, 'g-')
    axes[1, 0].set_title('RHS (mu0 * Jtor)')
    axes[1, 0].set_xlabel('rho')
    
    axes[1, 1].plot(rho, residual, 'k-')
    axes[1, 1].set_title('GS Residual')
    axes[1, 1].set_xlabel('rho')
    
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / 'solovev_math_verification.png', dpi=150)
    plt.close()
    
    return psi_bar, lap_psi, rhs, residual


def verify_analytic_derivatives():
    """Verify using analytical derivatives."""
    R0 = 1.0
    a = 0.5
    Ip = 2e5
    Beta0 = 0.5
    alpha_m = 1.0
    alpha_n = 2.0
    
    mu0 = 4 * np.pi * 1e-7
    L = Ip / (2 * np.pi * R0)
    
    psi_axis = mu0 * L * R0**2 / 2 * (1.0 + Beta0)
    
    nr = 500
    rho = np.linspace(1e-6, 0.999, nr)
    
    psi_bar = solovev_psi_bar(rho, alpha_m, alpha_n, Beta0)
    
    x = np.clip(1.0 - rho**2, 0.0, None)
    
    dpsi_dx = (1.0 + Beta0 * (alpha_n + 1) * x**alpha_n) / (1.0 + Beta0)
    d2psi_dx2 = (Beta0 * alpha_n * (alpha_n + 1) * x**(alpha_n - 1)) / (1.0 + Beta0)
    
    dpsi_drho = dpsi_dx * (-2.0 * rho)
    d2psi_drho2 = d2psi_dx2 * (4.0 * rho**2) + dpsi_dx * (-2.0)
    
    lap_psi_bar_rho = (1.0 / rho) * dpsi_drho + d2psi_drho2
    
    lap_psi = lap_psi_bar_rho * psi_axis / a**2
    
    psiN = psi_bar
    jtorshape = (1.0 - psiN**alpha_m)**alpha_n
    
    pprime_val = L * Beta0 / R0 * jtorshape
    ffprime_val = mu0 * L * (1.0 - Beta0) * R0 * jtorshape
    
    Jtor = R0 * pprime_val + ffprime_val / (mu0 * R0)
    rhs = mu0 * Jtor
    
    residual = -lap_psi - rhs
    
    print(f"\nAnalytical Derivatives Verification:")
    print(f"  Residual Statistics:")
    print(f"    Mean: {np.mean(residual):.6e}")
    print(f"    Max: {np.max(residual):.6e}")
    print(f"    Min: {np.min(residual):.6e}")
    print(f"    RMSE: {np.sqrt(np.mean(residual**2)):.6e}")
    
    return psi_bar, lap_psi, rhs, residual


def test_simple_case():
    """Test simple case: Beta0=0 (no pressure gradient)."""
    R0 = 1.0
    a = 0.5
    Ip = 2e5
    Beta0 = 0.0
    alpha_m = 1.0
    alpha_n = 1.0
    
    mu0 = 4 * np.pi * 1e-7
    L = Ip / (2 * np.pi * R0)
    
    psi_axis = mu0 * L * R0**2 / 2 * (1.0 + Beta0)
    
    nr = 500
    rho = np.linspace(1e-6, 0.999, nr)
    drho = rho[1] - rho[0]
    
    psi_bar = solovev_psi_bar(rho, alpha_m, alpha_n, Beta0)
    
    dpsi_drho = np.gradient(psi_bar, drho)
    d2psi_drho2 = np.gradient(dpsi_drho, drho)
    
    lap_psi_bar_rho = (1.0 / rho) * dpsi_drho + d2psi_drho2
    
    lap_psi = lap_psi_bar_rho * psi_axis / a**2
    
    psiN = psi_bar
    jtorshape = (1.0 - psiN**alpha_m)**alpha_n
    
    pprime_val = L * Beta0 / R0 * jtorshape
    ffprime_val = mu0 * L * (1.0 - Beta0) * R0 * jtorshape
    
    Jtor = R0 * pprime_val + ffprime_val / (mu0 * R0)
    rhs = mu0 * Jtor
    
    residual = -lap_psi - rhs
    
    print(f"\nSimple Case (Beta0=0, n=1):")
    print(f"  Residual Statistics:")
    print(f"    Mean: {np.mean(residual):.6e}")
    print(f"    Max: {np.max(residual):.6e}")
    print(f"    Min: {np.min(residual):.6e}")
    print(f"    RMSE: {np.sqrt(np.mean(residual**2)):.6e}")
    
    return psi_bar, lap_psi, rhs, residual


if __name__ == '__main__':
    OUTPUT_DIR.mkdir(exist_ok=True)
    verify_solovev_math()
    verify_analytic_derivatives()
    test_simple_case()